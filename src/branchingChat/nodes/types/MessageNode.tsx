...
import { useBranchingChatConfig } from '../../config'
...
function MessageNodeComponent({ node, shape }: NodeComponentProps<MessageNode>) {
	const editor = useEditor()
 	const { streamEndpoint, notify } = useBranchingChatConfig()

	const handleSend = useCallback(() => {
		const endpoint = streamEndpoint?.replace(/\/$/, '')
		if (!endpoint) {
			notify?.('Set a streaming endpoint in the plugin settings before sending a message.')
			return
		}
		const targetUrl = `${endpoint}/stream`
		...
		;(async () => {
			try {
				...
				if (!response.ok) {
					...
					const errorMessage = text || `Streaming request failed with status ${response.status}`
					notify?.(errorMessage)
					updateNode<MessageNode>(editor, shape, (node) => ({
						...node,
						assistantMessage: '',
				...
				if (!response.body) {
					notify?.('The streaming response was empty.')
					updateNode<MessageNode>(editor, shape, (node) => ({
						...node,
						assistantMessage: '',
					}))
				...
				console.error(e)
				notify?.('Failed to stream response. Check the developer console for details.')
				updateNode<MessageNode>(editor, shape, (node) => ({
					...node,
					assistantMessage: '',
				}))
	...
	}, [editor, notify, shape, streamEndpoint])
